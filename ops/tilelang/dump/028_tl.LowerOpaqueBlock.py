...
class Module:
    ...
    def main(X_handle: T.handle, gamma_handle: T.handle, beta_handle: T.handle, Y_handle: T.handle, Mean_handle: T.handle, Rstd_handle: T.handle):
        ...
        X_smem = T.alloc_buffer((1, 8192), "bfloat16", scope="shared.dyn")
        X_smem_1 = T.decl_buffer((1, 8192), "bfloat16", data=X_smem.data, scope="shared.dyn")
        ...
        with T.attr(i, "pragma_unroll_explicit", T.bool(False)):
            for i in T.unroll(4):
                for vec in T.vectorized(8):
                    X_smem_1[0, i * 2048 + tx * 8 + vec] = X[bx, i * 2048 + tx * 8 + vec]
        i_1 = T.int32()
        with T.attr(i_1, "pragma_unroll_explicit", T.bool(False)):
            for i_1 in T.unroll(4):
                for vec in T.vectorized(8):
                    G_smem_1[i_1 * 2048 + tx * 8 + vec] = gamma[i_1 * 2048 + tx * 8 + vec]
        i_2 = T.int32()
        with T.attr(i_2, "pragma_unroll_explicit", T.bool(False)):
            for i_2 in T.unroll(4):
                for vec in T.vectorized(8):
                    B_smem_1[i_2 * 2048 + tx * 8 + vec] = beta[i_2 * 2048 + tx * 8 + vec]
        i_3 = T.int32()
        with T.attr(i_3, "pragma_unroll_explicit", T.bool(False)):
            for i_3 in T.unroll(4):
                ...
                for vec_copy in T.vectorized(8):
                    X_smem_local_cast[vec_copy] = X_smem_1[0, i_3 * 2048 + tx * 8 + vec_copy]
                for vec in range(2):
        ...
        Mean[bx] = mean_row_1[0]
        Rstd[bx] = rstd_row_1[0]
        i_5 = T.int32()
        with T.attr(i_5, "pragma_unroll_explicit", T.bool(False)):
            for i_5 in T.unroll(4):
                ...
                for vec_copy in T.vectorized(8):
                    X_smem_1[0, i_5 * 2048 + tx * 8 + vec_copy] = X_smem_local_cast_1[vec_copy]
        i_6 = T.int32()
        T.attr(i_6, "pragma_unroll_explicit", T.bool(False))
        for i_6 in T.unroll(4):
            for vec in T.vectorized(8):
                Y[bx, i_6 * 2048 + tx * 8 + vec] = X_smem_1[0, i_6 * 2048 + tx * 8 + vec]