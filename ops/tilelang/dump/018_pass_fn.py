        ...
        with T.sblock("root"):
            ...
            with T.sblock("tilelang_root"):
                ...
                for i in T.unroll(4, annotations={"pragma_unroll_explicit": T.bool(False)}):
                    ...
                    for vec in T.vectorized(8):
                        X_local[i * 8 + vec] = T.Cast("float32", X_smem_local_cast[vec])
                ...
                for i in T.unroll(4, annotations={"pragma_unroll_explicit": T.bool(False)}):
                    ...
                    for vec in T.vectorized(8):
                        X_smem_local_cast_1[vec] = T.Cast("bfloat16", (X_local[i * 8 + vec] - mean_row[0]) * rstd_row[0] * T.Cast("float32", G_smem_local_cast_2[vec]) + T.Cast("float32", B_smem_local_cast_3[vec]))
                    for vec_copy in T.vectorized(8):
