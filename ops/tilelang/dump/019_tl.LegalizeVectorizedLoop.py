            ...
            with T.sblock("tilelang_root"):
                ...
                for i in T.unroll(4, annotations={"pragma_unroll_explicit": T.bool(False)}):
                    ...
                    for vec_copy in T.vectorized(8):
                        X_smem_local_cast[vec_copy] = X_smem[0, (i * 8 + vec_copy) // 8 * 2048 + tx * 8 + (i * 8 + vec_copy) % 8]
                    for vec in range(2):
                ...
                for i in T.unroll(4, annotations={"pragma_unroll_explicit": T.bool(False)}):
                    ...
                    for vec_copy in T.vectorized(8):
                        G_smem_local_cast_2[vec_copy] = G_smem[(i * 8 + vec_copy) // 8 * 2048 + tx * 8 + (i * 8 + vec_copy) % 8]
                    for vec_copy in T.vectorized(8):
                        B_smem_local_cast_3[vec_copy] = B_smem[(i * 8 + vec_copy) // 8 * 2048 + tx * 8 + (i * 8 + vec_copy) % 8]
                    for vec in range(2):
                        for vec_1 in T.vectorized(4):
                    ...
                    for vec_copy in T.vectorized(8):
                        X_smem[0, (i * 8 + vec_copy) // 8 * 2048 + tx * 8 + (i * 8 + vec_copy) % 8] = X_smem_local_cast_1[vec_copy]
                if tx < 256 and tx >= 0:
                    for i in T.unroll(4, annotations={"pragma_unroll_explicit": T.bool(False)}):
                        for vec in T.vectorized(8):
