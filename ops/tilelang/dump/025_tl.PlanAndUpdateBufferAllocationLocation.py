            ...
            with T.sblock("tilelang_root"):
                ...
                for i in T.unroll(4, annotations={"pragma_unroll_explicit": T.bool(False)}):
                    for vec in T.vectorized(8):
                        X_smem[0, i * 2048 + tx * 8 + vec] = X[bx, i * 2048 + tx * 8 + vec]
                for i in T.unroll(4, annotations={"pragma_unroll_explicit": T.bool(False)}):
                    for vec in T.vectorized(8):
                        G_smem[i * 2048 + tx * 8 + vec] = gamma[i * 2048 + tx * 8 + vec]
                for i in T.unroll(4, annotations={"pragma_unroll_explicit": T.bool(False)}):
                    for vec in T.vectorized(8):
                        B_smem[i * 2048 + tx * 8 + vec] = beta[i * 2048 + tx * 8 + vec]
                for i in T.unroll(4, annotations={"pragma_unroll_explicit": T.bool(False)}):
                    X_smem_local_cast = T.alloc_buffer((8,), "bfloat16", scope="local")
                    for vec_copy in T.vectorized(8):
                        X_smem_local_cast[vec_copy] = X_smem[0, i * 2048 + tx * 8 + vec_copy]
                    for vec in range(2):
                        for vec_1 in T.vectorized(4):
                            X_local[i * 8 + vec * 4 + vec_1] = T.Cast("float32", X_smem_local_cast[vec * 4 + vec_1])
                ...
                for i in T.unroll(1, annotations={"pragma_unroll_explicit": T.bool(False)}):
                    with T.sblock(""):
                        T.reads(sum_row[0], X_local[0:32])
                        T.writes(sum_row[0])
                ...
                for i in T.unroll(1, annotations={"pragma_unroll_explicit": T.bool(False)}):
                    with T.sblock(""):
                        T.reads(sumsq_row[0], X_sq_local[0:32])
                        T.writes(sumsq_row[0])
                        workspace = T.sblock_alloc_buffer((256,), scope="shared.dyn")
                        sumsq_row[0] = T.float32(0.0)
                        for rv in T.unroll(32, annotations={"pragma_unroll_explicit": T.bool(False)}):
                            sumsq_row[0] = sumsq_row[0] + X_sq_local[rv % 4 * 8 + rv // 4]
                        sumsq_row[0] = T.call_extern("float32", "tl::AllReduce<tl::SumOp, 256, 1, 0>::run", sumsq_row[0], T.tvm_access_ptr(T.type_annotation("float32"), workspace.data, 0, 256, 2))
                for i in T.unroll(1, annotations={"pragma_unroll_explicit": T.bool(False)}):
                ...
                for i in T.unroll(4, annotations={"pragma_unroll_explicit": T.bool(False)}):
                    ...
                    for vec_copy in T.vectorized(8):
                        G_smem_local_cast_2[vec_copy] = G_smem[i * 2048 + tx * 8 + vec_copy]
                    for vec_copy in T.vectorized(8):
                        B_smem_local_cast_3[vec_copy] = B_smem[i * 2048 + tx * 8 + vec_copy]
                    for vec in range(2):
                        for vec_1 in T.vectorized(4):
                            X_smem_local_cast_1[vec * 4 + vec_1] = T.Cast("bfloat16", (X_local[i * 8 + vec * 4 + vec_1] - mean_row[0]) * rstd_row[0] * T.Cast("float32", G_smem_local_cast_2[vec * 4 + vec_1]) + T.Cast("float32", B_smem_local_cast_3[vec * 4 + vec_1]))
                    for vec_copy in T.vectorized(8):
                        X_smem[0, i * 2048 + tx * 8 + vec_copy] = X_smem_local_cast_1[vec_copy]
                for i in T.unroll(4, annotations={"pragma_unroll_explicit": T.bool(False)}):
                    for vec in T.vectorized(8):
                        Y[bx, i * 2048 + tx * 8 + vec] = X_smem[0, i * 2048 + tx * 8 + vec]
