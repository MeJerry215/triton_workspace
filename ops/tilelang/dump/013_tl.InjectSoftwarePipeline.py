...
class Module:
    ...
    def main(X_handle: T.handle, gamma_handle: T.handle, beta_handle: T.handle, Y_handle: T.handle, Mean_handle: T.handle, Rstd_handle: T.handle):
        ...
        with T.sblock("tilelang_root"):
            ...
            inv_D: T.float32 = T.float32(0.0001220703125)
            for i in T.parallel(1):
                mean_row[0] = sum_row[0] * inv_D
                rstd_row[0] = T.rsqrt(sumsq_row[0] * inv_D - mean_row[0] * mean_row[0] + T.float32(1.0000000000000001e-05))
                Mean[bx] = mean_row[0]
                Rstd[bx] = rstd_row[0]
            for i in T.parallel(1):
                for j in T.parallel(8192):
