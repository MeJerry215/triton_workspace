...
class Module:
    ...
    def main_kernel(Mean: T.handle("float32", "global"), Rstd: T.handle("float32", "global"), X: T.handle("bfloat16", "global"), Y: T.handle("bfloat16", "global"), beta: T.handle("bfloat16", "global"), gamma: T.handle("bfloat16", "global")):
        ...
        for rv in T.unroll(32):
            sum_row_1[0] = sum_row_1[0] + X_local_1[T.bitwise_and(rv, 3) * 8 + T.shift_right(rv, 2)]
        T.tvm_storage_sync("shared.dyn")
        workspace_2 = T.Buffer((1,), data=workspace_1, scope="shared.dyn")
        sum_row_1[0] = T.call_extern("float32", "tl::AllReduce<tl::SumOp, 256, 1, 0>::run", sum_row_1[0], T.address_of(workspace_2[0]))
        sumsq_row_1[0] = T.float32(0.0)
        for rv in T.unroll(32):
            sumsq_row_1[0] = sumsq_row_1[0] + X_sq_local_1[T.bitwise_and(rv, 3) * 8 + T.shift_right(rv, 2)]
        T.tvm_storage_sync("shared.dyn")
        workspace_3 = T.Buffer((1,), data=workspace, scope="shared.dyn")
        sumsq_row_1[0] = T.call_extern("float32", "tl::AllReduce<tl::SumOp, 256, 1, 0>::run", sumsq_row_1[0], T.address_of(workspace_3[0]))
        mean_row_1[0] = sum_row_1[0] * T.float32(0.0001220703125)
        rstd_row_1[0] = T.call_pure_extern("float32", "rsqrtf", sumsq_row_1[0] * T.float32(0.0001220703125) - mean_row_1[0] * mean_row_1[0] + T.float32(1.0000000000000001e-05))
